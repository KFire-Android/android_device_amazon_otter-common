...
MEMPLUGIN_ERRORTYPE MemPlugin_ION_Init(void **pMemPluginHandle)
{
    ...
    if(pMemPluginHdl == NULL)
    {
        ...
        MEMPLUGIN_ERROR("%s: allocation failed",__FUNCTION__);
        goto EXIT;
    }
...
MEMPLUGIN_ERRORTYPE MemPlugin_ION_Open(void *pMemPluginHandle,OMX_U32 *pClient)
{
    ...
    if(memClient == 0)
    {
        MEMPLUGIN_ERROR("ion open failed");
                                    ...
                                    MEMPLUGIN_BUFFER_PROPERTIES *pIonBufferProp)
{
    ...
    if(pIonBufferParams->nWidth <= 0)
    {
        ...
        MEMPLUGIN_ERROR("%s: width should be positive %d", __FUNCTION__,pIonBufferParams->nWidth);
        goto EXIT;
    }
    ...
    if(pIonBufferParams->eBuffer_type == DEFAULT)
    {
        ...
        if(ret || (int)temp == -ENOMEM)
        {
            if(sIonParams.alloc_flags != OMAP_ION_HEAP_SECURE_INPUT)
            {
               ...
               MEMPLUGIN_ERROR("FAILED to allocate secure buffer of size=%d. ret=0x%x",pIonBufferParams->nWidth, ret);
               eError = MEMPLUGIN_ERROR_NORESOURCES;
               goto EXIT;
            }
            else
            {
                ...
                MEMPLUGIN_DEBUG("FAILED to allocate from non tiler space - trying tiler 1d space");
    ...
    if(pIonBufferParams->eBuffer_type == TILER1D)
    {
        ret = (OMX_S16)ion_alloc_tiler(nClient,
         ...
         if (ret || ((int)temp == -ENOMEM))
         {
               MEMPLUGIN_ERROR("FAILED to allocate buffer of size=%d. ret=0x%x",pIonBufferParams->nWidth, ret);
               eError = MEMPLUGIN_ERROR_NORESOURCES;
               goto EXIT;
         }
    }
    ...
    {
        MEMPLUGIN_ERROR("Tiler 2D not implemented");
        eError = MEMPLUGIN_ERROR_NOTIMPLEMENTED;
        goto EXIT;
    }
    else if(!temp)
    {
        MEMPLUGIN_ERROR("Undefined option for buffer type");
        eError = MEMPLUGIN_ERROR_UNDEFINED;
        goto EXIT;
    }
    ...
    if(pIonBufferParams->bMap == OMX_TRUE)
    {
        ...
        if(ret < 0)
        {
                MEMPLUGIN_ERROR("userspace mapping of ION buffers returned error");
                eError = MEMPLUGIN_ERROR_NORESOURCES;
                goto EXIT;
        }
    }
    else
    {
        ...
        if(ret < 0)
        {
                MEMPLUGIN_ERROR("ION share returned error");
                eError = MEMPLUGIN_ERROR_NORESOURCES;
                goto EXIT;
        }
    }
EXIT:
      if (eError != MEMPLUGIN_ERROR_NONE) {
          MEMPLUGIN_EXIT("%s exited with error 0x%x",__FUNCTION__,eError);
         return eError;
      }
      else {
          MEMPLUGIN_EXIT("%s executed successfully",__FUNCTION__);
...
{
    MEMPLUGIN_ERRORTYPE eError = MEMPLUGIN_ERROR_NONE;
    MEMPLUGIN_ENTER("%s: called with arguments ion fd %d buffer-params 0x%x buffer-prop 0x%x",__FUNCTION__,nClient,pIonBufferParams,pIonBufferProp);
